...
export function AdminRanking() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        setError('');
        Promise.all([dataService.getConfig(), dataService.getAllBookings()]).then(([config, records]) => {
            if (cancelled) return;
    ...
    const rows = useMemo(() => clubRankings(users, bookings, year, month), [users, bookings, year, month]);
